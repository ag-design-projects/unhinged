...
Reference asset:
`UNHINGED_MOBILE_UI_REFERENCE.png`

A separate mobile UI reference image is included alongside this spec. It shows the intended neo-brutalist visual treatment and the major mobile game screens. The reference artwork still uses the old working title in parts of the image; the implemented UI must use **UNHINGED**.

**Reference screens covered:** Home, Join Game, Lobby, Round Prompt/Answering, Voting, Vote Locked, Round Results, Ability Selection, Power Reveal, Round 2 with active power, Final Round, Final Results, and Rematch.
