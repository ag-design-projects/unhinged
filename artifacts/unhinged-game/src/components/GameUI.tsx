...
export function CTA({ children, onClick, disabled = false, testId, color = "yellow" }: { children: ReactNode; onClick?: () => void; disabled?: boolean; testId: string; color?: "yellow" | "pink" | "cyan" }) {
  return <Button data-testid={testId} type="button" buttonText={`${String(children)} →`} onClick={onClick} disabled={disabled} size="lg" color={color === "yellow" ? "yellow" : color === "pink" ? "pink" : "cyan"} style={{ width: "100%", fontFamily: "var(--unhinged-display)", fontSize: "1.05rem", letterSpacing: ".06em", boxShadow: disabled ? "none" : "4px 4px 0 #000" }} />;
}
