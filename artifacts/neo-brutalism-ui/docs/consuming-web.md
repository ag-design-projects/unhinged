# Consuming Neo Brutalism UI in web apps

Read `artifacts/neo-brutalism-ui/docs/AGENTS.md` first. This guide covers
...
`artifacts/neo-brutalism-ui/docs/migrating-web.md` before writing UI.
...
```css
@import "@workspace/neo-brutalism-ui/styles.css";
```
...
`node_modules/@workspace/neo-brutalism-ui/src/components` to `content`.
...
```tsx
import { Button } from "@workspace/neo-brutalism-ui/components/ui/button";
import { cn } from "@workspace/neo-brutalism-ui/lib/utils";
import {
  toast,
  useToast,
} from "@workspace/neo-brutalism-ui/hooks/use-toast";
```
...
`@workspace/neo-brutalism-ui/components/ui/button`. Run the app's typecheck
