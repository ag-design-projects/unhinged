...
workspace package (`@workspace/neo-brutalism-ui`): other artifacts depend
on it and import its theme and components directly.

## What's here

- `docs/references/component-inventory.md` — source-backed catalog, chunk plan,
  status, pinned commit, and provenance. Read it before adding a catalog family.
- `docs/references/components/*.md` — per-family build contracts derived from the
  canonical source. Read the matching file before modifying a component.
...
`@workspace/neo-brutalism-ui/components/...`. Never use a `@/` alias inside
...
for `@workspace/neo-brutalism-ui` and renders mockups using this package's
theme and components.

## Consuming this package

Never copy token values, component source, hooks, or these docs into a consuming
artifact. Add `@workspace/neo-brutalism-ui` as a `workspace:*` dependency,
...
- Building or styling web UI: `artifacts/neo-brutalism-ui/docs/consuming-web.md`
- Building or styling Expo UI: `artifacts/neo-brutalism-ui/docs/consuming-expo.md`
- Building or styling a slide deck: `artifacts/neo-brutalism-ui/docs/consuming-slides.md`
- Replacing an existing or scaffolded web theme/component library:
  `artifacts/neo-brutalism-ui/docs/migrating-web.md`
- Replacing existing or scaffolded Expo theme/hooks/components:
  `artifacts/neo-brutalism-ui/docs/migrating-expo.md`
...
- Match exact token values. Do not invent colors, fonts, spacing, or radii in a
  consuming app.
- Preserve the source's heavy black borders, flat high-contrast colors, and hard
  offset shadows. Square corners are the default; rounded variants are explicit.
- The source only defines a light mode. The artifact's dark mode is derived and
  must not be described as source-identical.
